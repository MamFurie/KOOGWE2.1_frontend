...
class _DriverWalletScreenState extends State<DriverWalletScreen> {
  ...
  void initState() {
    super.initState();
    if (widget.driverId.isNotEmpty) _loadData();
  }

  @override
  void didUpdateWidget(DriverWalletScreen old) {
    super.didUpdateWidget(old);
    if (old.driverId != widget.driverId && widget.driverId.isNotEmpty) _loadData();
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      final balance = await WalletService.getBalance(widget.driverId);
      final transactions = await WalletService.getTransactions(widget.driverId);
                        ...
                        Center(
                          child: Column(
                            children: [
                              ...
                              Text('${_balance.toStringAsFixed(2)} €', style: GoogleFonts.dmSans(fontSize: 40, fontWeight: FontWeight.w800, color: AppColors.textDark)),
                            ],
                          ),
                        ),
                        ...
                        if (_stats != null) ...[
                          Row(
                            children: [
                              ...
                              Expanded(child: _StatCard(label: 'TOTAL GAGNÉ', value: '${_stats!.totalEarnings.toStringAsFixed(0)} €', badge: 'Depuis le début', positive: true)),
                            ],
                          ),
                          const SizedBox(height: 24),
                        ],
