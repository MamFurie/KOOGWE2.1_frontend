...
class _OtpScreenState extends State<OtpScreen> {
  ...
  Future<void> _verify() async {
    ...
    try {
      ...
      // Note: En dev, le compte est directement vérifié à l'inscription
      // Donc on peut aussi tenter un login direct ici
      // Pour un vrai flux OTP, le login se fait après vérification
      ...
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(
          builder: (_) => widget.isPassenger ? const PassengerHomeScreen() : const DriverHomeScreen(),
        ),
        (route) => false,
      );
      ...
      // En mode dev, le compte est déjà activé — rediriger directement
      await SocketService.connect();
      if (!mounted) return;
      Navigator.pushAndRemoveUntil(
        context,
