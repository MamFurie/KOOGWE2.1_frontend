...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  String _selectedCountry = 'Brésil';
  String _selectedDialCode = '+55';
  bool _loading = false;
  String? _error;
  int _step = 0;
  ...
  Widget _buildFormStep() {
    return LayoutBuilder( // ← AJOUTÉ pour gérer la contrainte de hauteur
      builder: (context, constraints) {
        return SingleChildScrollView(
          padding: const EdgeInsets.symmetric(horizontal: 24),
          child: ConstrainedBox( // ← AJOUTÉ pour limiter la hauteur minimale
            constraints: BoxConstraints(
              minHeight: constraints.maxHeight,
            ),
            child: IntrinsicHeight( // ← AJOUTÉ pour ajuster au contenu
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const SizedBox(height: 16),
                  ...
                  Text(
                    widget.isPassenger ? 'Commencez à réserver vos courses' : 'Commencez à conduire et à gagner',
                    style: GoogleFonts.dmSans(fontSize: 14, color: AppColors.textLight, height: 1.5),
                  ),
                  const SizedBox(height: 28),
                  KoogweInput(label: 'Nom complet', hint: 'ex. Juan Pérez', prefixIcon: Icons.person_outline, controller: _nameCtrl),
                  const SizedBox(height: 16),
                  KoogweInput(label: 'Adresse email', hint: 'nom@exemple.com', prefixIcon: Icons.mail_outline, controller: _emailCtrl, keyboardType: TextInputType.emailAddress),
                  const SizedBox(height: 16),
                  // Country + Phone
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Téléphone', style: GoogleFonts.dmSans(fontSize: 14, fontWeight: FontWeight.w500, color: AppColors.textDark)),
                      const SizedBox(height: 8),
                      Row(
                        children: [
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Row(
                                children: [
                                  Text(_selectedDialCode, style: GoogleFonts.dmSans(fontSize: 15, fontWeight: FontWeight.w500, color: AppColors.textDark)),
                  ...
                  const SizedBox(height: 16),
                  KoogweInput(label: 'Mot de passe', hint: '••••••••', obscure: true, prefixIcon: Icons.lock_outline, controller: _passCtrl),
                  if (_error != null) ...[
                    const SizedBox(height: 12),
                    Container(
                  ...
                  const Spacer(), // ← AJOUTÉ pour pousser le bouton vers le bas
                  KoogweButton(
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        ...
        child: Column(
          ...
          children: [
            Text('Indicatif pays', style: GoogleFonts.dmSans(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.textDark)),
            const SizedBox(height: 16),
            ..._countries.map((c) => ListTile(
              onTap: () {
                setState(() { _selectedCountry = c['name']!; _selectedDialCode = c['code']!; });
