...
class _WalletScreenState extends State<WalletScreen> {
  ...
  void initState() {
    super.initState();
    if (widget.userId.isNotEmpty) _loadData();
  }

  @override
  void didUpdateWidget(WalletScreen old) {
    super.didUpdateWidget(old);
    if (old.userId != widget.userId && widget.userId.isNotEmpty) _loadData();
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      final balance = await WalletService.getBalance(widget.userId);
      final txRaw = await WalletService.getTransactions(widget.userId);
      if (mounted) {
        setState(() {
          _balance = balance;
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  Text('${_balance.toStringAsFixed(2)} €', style: GoogleFonts.dmSans(fontSize: 36, fontWeight: FontWeight.w800, color: Colors.white)),
                                  const Spacer(),
                                  GestureDetector(
                                    onTap: () {},
                                    child: Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(transaction.formattedDate, style: GoogleFonts.dmSans(fontSize: 11, color: AppColors.textLight)),
              ],
            ),
          ),
          Column(
